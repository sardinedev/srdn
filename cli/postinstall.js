...
let binary = process.platform === 'win32' ? 'srdn.exe' : 'srdn';

let pkgPath;
try {
  pkgPath = path.dirname(require.resolve(`@sardine/srdn-cli-${parts.join('-')}/package.json`));
} catch (err) {
...
if (process.platform === 'win32') {
  try {
    fs.unlinkSync(path.join(__dirname, 'srdn'));
  } catch (err) {}
}